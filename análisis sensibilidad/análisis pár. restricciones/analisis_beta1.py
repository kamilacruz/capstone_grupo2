...
if __name__ == "__main__":
    ...
    # Valores de incremento sobre los coeficientes B1
    deltas_b1 = [-5e-4, -5e-2, 0.005, 0.05, 0.5, 5]
    resultados_escenarios = []

    for delta in deltas_b1:
        nombre_escenario = f"DeltaB1_{delta:.0e}"
        ...
        for iteracion in range(iteraciones):
            inicio = time.time()
            result = correr_modelo(delta_B1=delta)
        ...
        if not utilidades:
            print(f"\n⚠️ No se obtuvieron soluciones para delta_B1 = {delta}")
            continue
    ...
    df_resumen.to_csv("resumen_analisis_delta_B1.csv", index=False)
    print("\nResultados guardados en resumen_analisis_delta_B1.csv")
    print(df_resumen)
