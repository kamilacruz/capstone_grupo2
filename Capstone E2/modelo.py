...
def correr_modelo():
    ...
    m.addConstrs(p[i, 1, t] - p[i, 2, t] <= 3.8 for i in I_ for t in T_)
    m.addConstrs(p[i, 2, t] - p[i, 1, t] <= 3.8 for i in I_ for t in T_)
    ...
    m.addConstrs(di[i, j, t] == A[i, j, t] - B[i, j, t] * p[i, j, t] - v[i, j, t] for i in I_ for j in J_ for t in T_)

    # 7. Venta del producto Vijt <= Dijt(pijt), Vijt <= Iijt
    m.addConstrs(v[i, j, t] <= A[i, j, t] - B[i, j, t] * p[i, j, t] for i in I_ for j in J_ for t in T_)
    m.addConstrs(v[i, j, t] <= inv[i, j, t] for i in I_ for j in J_ for t in T_)    

    # 8. Iijt+1 = Iijt - Vijt + Qijt	
    m.addConstrs(inv[i, j, t+1] == inv[i, j, t] - v[i, j, t] + q[i, j, t] for i in I_ for j in J_ for t in T_[:-1])   
